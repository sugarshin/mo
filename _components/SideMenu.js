    ...
    <aside className={classnames('global-sidemenu', { open: isOpen })}>
      ...
      <ListGroup>
        {router.pathname === '/build'
          ? [
              <ListGroupItem key="rebuild" onClick={rebuild}>
                Rebuild
              </ListGroupItem>,
              <ListGroupItem key="cancel" onClick={cancelBuild}>
                Cancel
              </ListGroupItem>,
              <ListGroupItem
                key="rebuild-withou-cache"
                onClick={rebuildWithoutCache}
              >
      ...
      <div className="copyright">
        <small>
          {`@ ${getYear(new Date())} mo | `}
          <a href="https://sugarshin.net/" target="_blank">
            @sugarshin
          </a>
        </small>
      </div>
