...
class Top extends PureComponent {
  static async getInitialProps({
    ...
    await dispatch(
      fetchRecentBuilds({
        req,
        query: { offset: query && (query.page ? query.page : 0) * 20 }
      })
    )
    if (isServer) {
      ...
      <Main isServer={this.props.isServer}>
        ...
        <ListGroup className="build-list">
          {this.props.builds.map(b => (
            <ListGroupItem
              key={`${b.vcs_type}-${b.vcs_revision}-${b.build_num}`}
            >
                ...
                <a className="cell">
                  ...
                  <div className="content">
                    ...
                    <ListGroupItemText tag="div">
                      <Octicon name="git-branch" className="branch-icon" />
                      <span>{`${b.branch} (${b.vcs_revision.slice(
                        0,
                        7
                      )})`}</span>
                    </ListGroupItemText>
