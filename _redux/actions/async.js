...
export const fetchMe = ({ req } = {}) => async (
  dispatch,
  getState,
  { cookies }
) => {
  ...
  try {
    payload = await request(
      `${getApiRoot(req)}/api/v1.1/me?circle-token=${cookies.get(
        CIRCLECI_TOKEN
      )}`
    )
  } catch (e) {
...
export const fetchRecentBuilds = ({ req, query = {} } = {}) => async (
  dispatch,
  getState,
  { cookies }
) => {
  ...
  try {
    ...
    payload = await request(
      `${getApiRoot(req)}/api/v1.1/recent-builds?${queryString}`
    )
  } catch (e) {
    payload = e
  } finally {
...
export const fetchSingleBuild = ({ req, url } = {}) => async (
  dispatch,
  getState,
  { cookies }
) => {
  ...
  try {
    payload = await request(
      `${getApiRoot(req)}/api/v1.1/project${resolveVcsType(
        url
      )}?circle-token=${cookies.get(CIRCLECI_TOKEN)}`
    )
  } catch (e) {
...
export const pollFetchSingleBuild = ({ req, url } = {}) => async (
  dispatch,
  getState,
  { cookies }
) => {
...
export const fetchSteps = data => async (dispatch, getState, { cookies }) => {
  ...
  try {
    const res = await Promise.all(
      data.map(
        ({ url }) => (url ? request(url, {}, 'text') : Promise.resolve())
      )
    )
    payload = data.map((d, i) => ({
      ...d,
      message: res[i]
        ? JSON.parse(res[i]).reduce((res, r) => `${res}${r.message}`, '')
        : ''
    }))
  } catch (e) {
    payload = e
...
export const rebuild = url => async (dispatch, getState, { cookies }) => {
  ...
  try {
    payload = await request(
      `/api/v1.1/project${resolveVcsType(url)}/retry?circle-token=${cookies.get(
        CIRCLECI_TOKEN
      )}`,
      { method: 'POST' }
    )
...
export const cancelBuild = url => async (dispatch, getState, { cookies }) => {
  ...
  try {
    payload = await request(
      `/api/v1.1/project${resolveVcsType(
        url
      )}/cancel?circle-token=${cookies.get(CIRCLECI_TOKEN)}`,
      { method: 'POST' }
    )
  } catch (e) {
    payload = e
...
export const deleteBuildCache = url => async (
  dispatch,
  getState,
  { cookies }
) => {
  ...
  try {
    payload = await request(
      `/api/v1.1/project${resolveVcsType(
        url
      )}/build-cache?circle-token=${cookies.get(CIRCLECI_TOKEN)}`,
      { method: 'DELETE' }
    )
  } catch (e) {
    payload = e
...
export const rebuildWithoutCache = url => async (
  dispatch,
  getState,
  { cookies }
) => {
  ...
  try {
    await request(
      `/api/v1.1/project${resolvedUrl.replace(
        /\/[0-9]+$/,
        ''
      )}/build-cache?circle-token=${cookies.get(CIRCLECI_TOKEN)}`,
      { method: 'DELETE' }
    ...
    payload = await request(
      `/api/v1.1/project${resolvedUrl}/retry?circle-token=${cookies.get(
        CIRCLECI_TOKEN
      )}`,
      { method: 'POST' }
    )
